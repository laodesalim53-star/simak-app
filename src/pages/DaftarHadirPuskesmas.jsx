...
const KOLOM_RELASI_PEGAWAI = 'pegawai_puskesmas_id'
const KOLOM_TANGGAL = 'tanggal'
const KOLOM_STATUS = 'status'
const KOLOM_KETERANGAN = 'keterangan'
const KOLOM_JAM_MASUK = 'jam_masuk'
const KOLOM_JAM_PULANG = 'jam_pulang'

const TABEL_HARI_LIBUR = 'hari_libur'
const KOLOM_TANGGAL_LIBUR = 'tanggal'

const SINGKATAN_STATUS = {
  hadir: 'H', izin: 'I', sakit: 'S', alpa: 'A', cuti: 'C', dinas: 'D',
}
...
export default function DaftarHadirPuskesmas() {
  ...
  const [tanggalLibur, setTanggalLibur] = useState(new Set())
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select('nama_puskesmas, alamat, kabupaten, kecamatan, telepon, email, kepala_puskesmas, nip_kepala_puskesmas, tempat_ttd, logo_path')
  ...
  useEffect(() => {
    async function muatData() {
      ...
      if (error) {
        console.warn('Kolom jam tidak lengkap, mencoba tanpa kolom jam:', error.message)
        ...
        if (tanpaJam.error) {
          ...
          presensi = dasar.data
          if (dasar.error) {
            console.error('Gagal memuat presensi:', dasar.error)
          }
        }
      }
  ...
  const tempatTtd = profilPuskesmas?.tempat_ttd || profilPuskesmas?.kabupaten || ''
  ...
  const unitKerja = profilPuskesmas?.nama_puskesmas || '-'
  ...
  return (
    <Layout
      ...
      subtitle="Rekap kehadiran pegawai puskesmas per bulan, otomatis dari data pegawai, siap cetak."
    >
      <div className="no-print flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex flex-wrap items-center gap-2">
          <select value={bulan} onChange={(e) => setBulan(Number(e.target.value))} className="border border-slate-300 rounded-lg px-3 py-2 text-sm">
            {NAMA_BULAN.map((nama, i) => (
              <option key={nama} value={i + 1}>{nama}</option>
            ))}
          </select>
          <select value={tahun} onChange={(e) => setTahun(Number(e.target.value))} className="border border-slate-300 rounded-lg px-3 py-2 text-sm">
          ...
          <div className="flex items-center rounded-lg border border-slate-300 overflow-hidden">
            <button type="button" onClick={() => setMode('kolektif')} className={`px-3 py-2 text-sm font-medium transition-colors ${mode === 'kolektif' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>
              Kolektif
            </button>
            <button type="button" onClick={() => setMode('perorangan')} className={`px-3 py-2 text-sm font-medium transition-colors border-l border-slate-300 ${mode === 'perorangan' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>
              Perorangan
            </button>
          </div>

          {mode === 'perorangan' && (
            <select value={pegawaiTerpilihId} onChange={(e) => setPegawaiTerpilihId(e.target.value)} className="border border-slate-300 rounded-lg px-3 py-2 text-sm max-w-[220px]">
              {pegawaiList.map((p) => (
        ...
        <button onClick={() => window.print()} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
      ...
      {loading ? (
        ...
        <div className="lembar-cetak print-only bg-white rounded-2xl border border-slate-100 p-5 sm:p-8 mx-auto" style={{ width: '297mm' }}>
          <KopSurat />
          ...
          <table className="w-full border-collapse text-[9px] sm:text-[10px]">
            <thead>
              <tr className="bg-slate-100">
                ...
                {daftarHari.map((hari) => (
                  <th key={hari} className={`border border-slate-400 px-0.5 py-1 w-4 ${apakahLibur(hari) ? 'text-red-600' : ''}`}>
                    {hari}
                  </th>
                ))}
              ...
              {pegawaiList.map((pegawai, i) => {
                ...
                return (
                  <tr key={pegawai.id}>
                    ...
                    {daftarHari.map((hari) => (
                      <td key={hari} onClick={() => bukaEditPresensi(pegawai.id, hari)} title="Klik untuk edit presensi" className={`no-print-cursor border border-slate-300 text-center cursor-pointer hover:bg-emerald-50 ${apakahLibur(hari) ? 'text-red-600' : ''}`}>
          ...
          <div className="ttd-block flex justify-between mt-10 text-sm text-slate-700">
            <div className="text-center w-48">
              ...
              <p className="text-xs text-slate-500">
                NIP. {profilPuskesmas?.nip_kepala_puskesmas || '..............................'}
              </p>
            </div>
            <div className="text-center w-48">
        ...
        <div className="lembar-cetak lembar-perorangan print-only bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 mx-auto" style={{ width: '190mm' }}>
          <KopSurat />
            ...
            <>
              <table className="text-[11px] text-slate-800 mb-2">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="align-top pr-2 py-[1px]">:</td>
                    <td className="align-top py-[1px]">{NAMA_BULAN[bulan - 1]} {tahun}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="w-full border-collapse text-[10px] table-fixed">
                <thead>
                  <tr className="bg-slate-100 text-center">
                    ...
                    <th rowSpan={2} className="border border-slate-500 px-1 py-1 w-[70px] align-middle">Tanggal</th>
                ...
                <tbody>
                  {daftarHari.map((hari) => {
                    ...
                    if (libur) {
                      return (
                        <tr key={hari} onClick={() => bukaEditPresensi(pegawaiTerpilih.id, hari)} title="Klik untuk edit presensi" className="no-print-cursor text-red-600 font-semibold cursor-pointer hover:bg-emerald-50">
                          <td className="border border-slate-400 text-center h-[19px]">{hari}</td>
                          <td className="border border-slate-400 text-center whitespace-nowrap">{tanggalStr}</td>
                          <td colSpan={5} className="border border-slate-400 text-center tracking-wide">{libur}</td>
                        </tr>
                      )
                    }

                    return (
                      <tr key={hari} onClick={() => bukaEditPresensi(pegawaiTerpilih.id, hari)} title="Klik untuk edit presensi" className="no-print-cursor cursor-pointer hover:bg-emerald-50">
                        <td className="border border-slate-400 text-center h-[19px]">{hari}</td>
                        <td className="border border-slate-400 text-center whitespace-nowrap">{tanggalStr}</td>
                        ...
                        <td className="border border-slate-400 text-center text-[9px] truncate">
                          {dataHari?.keterangan || (dataHari && dataHari.singkatan !== 'H' ? kapital(dataHari.statusRaw) : '')}
                        </td>
                      </tr>
                    )
                  })}
              ...
              <div className="ttd-block flex justify-end mt-4 text-[11px] text-slate-800">
                <div className="w-64 text-left">
                  ...
                  <p>{unitKerja}</p>
                  <div className="h-14" />
                  <p className="font-semibold">{profilPuskesmas?.kepala_puskesmas || '..............................'}</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-5">
            ...
            <form onSubmit={simpanPresensi} className="space-y-3">
              <div>
                ...
                <select value={formEdit.status} onChange={(e) => setFormEdit((f) => ({ ...f, status: e.target.value }))} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" value={formEdit.jamMasuk} onChange={(e) => setFormEdit((f) => ({ ...f, jamMasuk: e.target.value }))} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Jam Pulang</label>
                  <input type="time" value={formEdit.jamPulang} onChange={(e) => setFormEdit((f) => ({ ...f, jamPulang: e.target.value }))} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Keterangan</label>
                <input type="text" value={formEdit.keterangan} onChange={(e) => setFormEdit((f) => ({ ...f, keterangan: e.target.value }))} placeholder="opsional" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
              </div>

              <div className="flex items-center justify-between pt-2">
                <button type="button" onClick={hapusPresensi} disabled={menyimpan} className="text-sm text-red-600 hover:text-red-700 disabled:opacity-50">
                  Hapus
                </button>
                <div className="flex gap-2">
                  <button type="button" onClick={tutupEditPresensi} disabled={menyimpan} className="px-4 py-2 text-sm rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                    Batal
                  </button>
                  <button type="submit" disabled={menyimpan} className="px-4 py-2 text-sm rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50">
      ...
      <style>{`
        ...
        @media screen {
          .lembar-cetak.print-only { display: block !important; overflow-x: auto; }
        }
        @media print {
          .no-print { display: none !important; }
          body { background: white; }
          .lembar-cetak { box-shadow: none !important; width: 297mm !important; max-width: 100% !important; margin-left: auto !important; margin-right: auto !important; }
          .lembar-perorangan { page: perorangan; width: 190mm !important; padding: 4mm 6mm !important; border-radius: 0 !important; }
          .lembar-perorangan table { page-break-inside: auto; }
          .lembar-perorangan tr { page-break-inside: avoid; }
          .ttd-block { page-break-inside: avoid; break-inside: avoid; }
          .no-print-cursor { cursor: default !important; }
        }
        @page { size: A4 landscape; margin: 10mm; }
        @page perorangan { size: A4 portrait; margin: 6mm; }
      `}</style>
    </Layout>
  )
}
