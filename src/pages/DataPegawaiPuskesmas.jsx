...
import { Plus, Pencil, Trash2, Search, X, Loader2, HeartPulse } from 'lucide-react'

// Halaman "Data Pegawai" KHUSUS tenant puskesmas — menulis ke tabel
// `pegawai_puskesmas` (migrasi-pegawai-puskesmas.sql), BUKAN ke `guru`
// atau `pegawai_kantor`. Pola disalin dari DataPegawaiKantor.jsx, tapi
// disederhanakan (tanpa fitur "Isi dari SK" dan field Detail SK &
// Penempatan) karena pegawai puskesmas tidak selalu punya SK sekompleks itu.
const emptyForm = {
  nama_lengkap: '',
  nip: '',
  nik: '',
...
export default function DataPegawaiPuskesmas() {
  ...
  async function handleSubmit(e) {
    ...
    const { error } = editingId
      ? await supabase.from('pegawai_puskesmas').update(payload).eq('id', editingId).eq('sekolah_id', sekolahId)
      : await supabase.from('pegawai_puskesmas').insert(payload)
    setSaving(false)
    if (!error) {
      setShowForm(false)
      loadData()
  ...
  async function handleDelete(id) {
    ...
    const { error } = await supabase.from('pegawai_puskesmas').delete().eq('id', id).eq('sekolah_id', sekolahId)
    if (!error) loadData()
            ...
            {filtered.map((p) => (
              <tr key={p.id} className="hover:bg-emerald-600/[0.03] transition-colors">
                <td className="font-medium">
                  <button type="button" onClick={() => setProfilLihat(p)} className="hover:underline hover:text-emerald-700 text-left">
                    {p.nama_lengkap}
                  </button>
                </td>
                ...
                <td>
                  <span className={`badge ${p.status === 'aktif' ? 'bg-emerald-600/15 text-emerald-700' : 'bg-red-900/10 text-red-900'}`}>
                    {p.status}
                  </span>
                </td>
                <td>
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => openEdit(p)} className="p-2 hover:bg-emerald-600/10 rounded-lg text-emerald-700/70">
                      <Pencil size={15} />
                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/50 backdrop-blur-sm p-4">
          <form onSubmit={handleSubmit} className="card relative overflow-hidden w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <SeksiForm judul="Kepegawaian">
              ...
              <Field label="Jabatan"><input className="input-field" placeholder="Perawat / Bidan / Dokter..." value={form.jabatan} onChange={(e) => setForm({ ...form, jabatan: e.target.value })} /></Field>
      ...
      {profilLihat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/50 backdrop-blur-sm p-4">
          <div className="card w-full max-w-md p-0 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative bg-gradient-to-br from-emerald-800 to-blue-900 pt-8 pb-16 flex flex-col items-center">
...
function SeksiForm({ judul, children }) {
  return (
    <div className="mt-5 first:mt-0">
      <p className="eyebrow text-emerald-700 mb-2">{judul}</p>
...
function SeksiProfil({ judul, children }) {
  return (
    <div>
      <p className="eyebrow text-emerald-700/70 mb-2">{judul}</p>
