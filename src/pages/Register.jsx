...
export default function Register() {
  ...
  const [jenisInstitusi, setJenisInstitusi] = useState('sekolah') // 'sekolah' | 'kantor'
      ...
      <div className="relative min-h-screen flex items-center justify-center px-4">
        ...
        <div className="relative max-w-sm w-full text-center bg-slate-900/70 backdrop-blur-xl p-7 rounded-2xl border border-blue-400/30 shadow-[0_0_40px_-10px_rgba(59,130,246,0.35)]">
          ...
          <p className="text-sm text-blue-200/70 mt-2">
            {mode === 'baru'
              ? `${isKantor ? 'Kantor' : 'Sekolah'} dan akun Anda sudah dibuat, sedang menunggu persetujuan Superadmin. Anda akan bisa login setelah disetujui.`
              : isOrangTua
                ...
                : `Akun Anda sudah dibuat dan sedang menunggu persetujuan admin ${isKantor ? 'kantor' : 'sekolah'}. Anda akan bisa login setelah disetujui.`}
          </p>
  ...
  async function handleSubmit(e) {
    ...
    if (mode === 'gabung' && !form.sekolahId) {
      setError(`Pilih ${isKantor ? 'kantor' : 'sekolah'} terlebih dahulu.`)
      return
    }
    ...
    <div className="relative min-h-screen flex items-center justify-center px-4 py-10">
      ...
      <div className="relative w-full max-w-4xl flex flex-col lg:flex-row items-stretch justify-center gap-4">
        ...
        >
          ...
          {/* Toggle jenis institusi: Sekolah / Kantor. Ini menentukan
              jabatan yang tersedia dan daftar institusi yang dimuat. */}
          <div>
            <label className={labelClass}>Jenis Institusi</label>
            <div className="grid grid-cols-2 gap-2 p-1 bg-slate-900/60 border border-blue-500/25 rounded-lg text-xs font-medium">
              <button
                type="button"
                onClick={() => ubahJenisInstitusi('sekolah')}
                className={`py-2 rounded-md transition ${
                  jenisInstitusi === 'sekolah' ? 'bg-blue-500/80 text-white' : 'text-blue-200/60 hover:text-blue-200'
                }`}
              >
                Sekolah
              </button>
              <button
                type="button"
                onClick={() => ubahJenisInstitusi('kantor')}
                className={`py-2 rounded-md transition ${
                  jenisInstitusi === 'kantor' ? 'bg-blue-500/80 text-white' : 'text-blue-200/60 hover:text-blue-200'
                }`}
              >
                Kantor
              </button>
          ...
          <div>
            ...
            {mode === 'baru' && !isOrangTua && (
              <p className="text-[11px] text-blue-200/50 mt-1">
                Karena mendaftarkan {isKantor ? 'kantor' : 'sekolah'} baru, akun Anda otomatis jadi Admin Utama {isKantor ? 'kantor' : 'sekolah'} ini.
              </p>
            )}
            {isOrangTua && (
              <p className="text-[11px] text-blue-200/50 mt-1">
          ...
          {!isOrangTua && (
            <div className="grid grid-cols-2 gap-2 p-1 bg-slate-900/60 border border-blue-500/25 rounded-lg text-xs font-medium">
              ...
              >
                Gabung {isKantor ? 'Kantor' : 'Sekolah'}
              </button>
              <button
                type="button"
              ...
              >
                Daftar {isKantor ? 'Kantor' : 'Sekolah'} Baru
              </button>
            </div>
          )}
          ...
          {mode === 'gabung' || isOrangTua ? (
            <div>
              <label className={labelClass}>{isKantor ? 'Kantor' : 'Sekolah'}</label>
              <select required className={inputClass}
                value={form.sekolahId} onChange={(e) => ubah('sekolahId', e.target.value)}>
                <option value="" className="bg-slate-900">-- Pilih {isKantor ? 'Kantor' : 'Sekolah'} --</option>
            ...
            <div>
              <label className={labelClass}>Nama {isKantor ? 'Kantor' : 'Sekolah'} Baru</label>
              <input required className={inputClass}
                placeholder={isKantor ? 'Contoh: Kantor Cabang Contoh' : 'Contoh: SD Negeri Contoh'}
