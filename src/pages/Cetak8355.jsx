...
export default function Cetak8355() {
  ...
  async function muatSemua() {
    ...
    // PERBAIKAN: dulu kelas 6 difilter langsung di query lewat
    // .ilike('kelas.nama_kelas', '6%'), sehingga kelas Romawi ("VIA",
    // "Kelas VI") tidak pernah ikut kebaca. Sekarang SEMUA siswa sekolah
    // diambil dulu (join kelas, left join biasa — bukan !inner — supaya
    // siswa tanpa kelas pun tidak bikin query gagal), lalu difilter di sisi
    // client memakai isKelas6() yang mengenali kedua format penamaan.
    ...
    // Urutkan per kelas dulu (alfabetis, cukup untuk kebutuhan lampiran ini),
    // baru per nama — menggantikan .order('kelas(nama_kelas)') di query lama
    // yang jadi tidak relevan lagi karena penyaringan kelas 6 sekarang
    // dilakukan di client, bukan di query.
    const kelas6 = (siswaRows || [])
      .filter((s) => isKelas6(s.kelas?.nama_kelas))
      .sort((a, b) => {
        const kelasA = a.kelas?.nama_kelas || ''
        const kelasB = b.kelas?.nama_kelas || ''
        if (kelasA !== kelasB) return kelasA.localeCompare(kelasB, 'id')
